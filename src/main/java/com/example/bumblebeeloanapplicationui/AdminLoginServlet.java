...
public class AdminLoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        adminJson.addProperty("username", username);
        adminJson.addProperty("password", password);

        // Send a POST request to the backend resource file
        URL url = new URL("http://localhost:8080/BB_REST_APP-1.0-SNAPSHOT/api/authadmin");
        ...
        try (OutputStream os = connection.getOutputStream()) {
            os.write(adminJson.toString().getBytes());
            os.flush();
        }
...
//            session.setAttribute("admin", username);
            ...
            response.getWriter().println("<script>window.location.href='AdminDashboard.jsp';</script>");
